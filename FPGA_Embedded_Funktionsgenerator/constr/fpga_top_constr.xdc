###############################################################################
# Constraints for FPGA-Embedded-Functiongenerator Design
# FPGA device/board: Xilinx Artix-7 on Digilent Basys3 board
# Author: Jonas Berger
# Date: 10.02.2023
###############################################################################


##USB-RS232 Interface
#set_property PACKAGE_PIN B18 [get_ports RsRx]						
#set_property IOSTANDARD LVCMOS33 [get_ports RsRx]
#set_property PACKAGE_PIN A18 [get_ports RsTx]						
#set_property IOSTANDARD LVCMOS33 [get_ports RsTx]


##USB HID (PS/2)
#set_property PACKAGE_PIN C17 [get_ports PS2Clk]						
#set_property IOSTANDARD LVCMOS33 [get_ports PS2Clk]
#set_property PULLUP true [get_ports PS2Clk]
#set_property PACKAGE_PIN B17 [get_ports PS2Data]					
#set_property IOSTANDARD LVCMOS33 [get_ports PS2Data]	
#set_property PULLUP true [get_ports PS2Data]


##Quad SPI Flash
##Note that CCLK_0 cannot be placed in 7 series devices. You can access it using the
##STARTUPE2 primitive.
#set_property PACKAGE_PIN D18 [get_ports {QspiDB[0]}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {QspiDB[0]}]
#set_property PACKAGE_PIN D19 [get_ports {QspiDB[1]}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {QspiDB[1]}]
#set_property PACKAGE_PIN G18 [get_ports {QspiDB[2]}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {QspiDB[2]}]
#set_property PACKAGE_PIN F18 [get_ports {QspiDB[3]}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {QspiDB[3]}]
#set_property PACKAGE_PIN K19 [get_ports QspiCSn]					
#set_property IOSTANDARD LVCMOS33 [get_ports QspiCSn]


##Pmod Header JA; JA1:J1=A, JA2:L2=B, JA3:J2=SW -> rotenc, JA4:G2=BTN
##Sch name = JA1
set_property PACKAGE_PIN J1 [get_ports {rotenc_i[1]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {rotenc_i[1]}]
##Sch name = JA2
set_property PACKAGE_PIN L2 [get_ports {rotenc_i[2]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {rotenc_i[2]}]
##Sch name = JA3
set_property PACKAGE_PIN J2 [get_ports {rotenc_i[0]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {rotenc_i[0]}]
##Sch name = JA4
set_property PACKAGE_PIN G2 [get_ports {rotenc_i[3]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {rotenc_i[3]}]
##Sch name = JA7
#set_property PACKAGE_PIN H1 [get_ports {JA[4]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JA[4]}]
##Sch name = JA8
#set_property PACKAGE_PIN K2 [get_ports {JA[5]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JA[5]}]
##Sch name = JA9
#set_property PACKAGE_PIN H2 [get_ports {JA[6]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JA[6]}]
##Sch name = JA10
#set_property PACKAGE_PIN G3 [get_ports {JA[7]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JA[7]}]


##Pmod Header JB
##Sch name = JB1
set_property PACKAGE_PIN A14 [get_ports {signal_o[0]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[0]}]
##Sch name = JB2
set_property PACKAGE_PIN A16 [get_ports {signal_o[1]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[1]}]
##Sch name = JB3
set_property PACKAGE_PIN B15 [get_ports {signal_o[2]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[2]}]
##Sch name = JB4
set_property PACKAGE_PIN B16 [get_ports {signal_o[3]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[3]}]
##Sch name = JB7
set_property PACKAGE_PIN A15 [get_ports {signal_o[4]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[4]}]
##Sch name = JB8
set_property PACKAGE_PIN A17 [get_ports {signal_o[5]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[5]}]
##Sch name = JB9
set_property PACKAGE_PIN C15 [get_ports {signal_o[6]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[6]}]
##Sch name = JB10 
set_property PACKAGE_PIN C16 [get_ports {signal_o[7]}]					
set_property IOSTANDARD LVCMOS33 [get_ports {signal_o[7]}]
 

##Pmod Header JC
##Sch name = JC1
#set_property PACKAGE_PIN K17 [get_ports {JC[0]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[0]}]
##Sch name = JC2
#set_property PACKAGE_PIN M18 [get_ports {JC[1]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[1]}]
##Sch name = JC3
#set_property PACKAGE_PIN N17 [get_ports {JC[2]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[2]}]
##Sch name = JC4
#set_property PACKAGE_PIN P18 [get_ports {JC[3]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[3]}]
##Sch name = JC7
#set_property PACKAGE_PIN L17 [get_ports {JC[4]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[4]}]
##Sch name = JC8
#set_property PACKAGE_PIN M19 [get_ports {JC[5]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[5]}]
##Sch name = JC9
#set_property PACKAGE_PIN P17 [get_ports {JC[6]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[6]}]
##Sch name = JC10
#set_property PACKAGE_PIN R18 [get_ports {JC[7]}]					
#set_property IOSTANDARD LVCMOS33 [get_ports {JC[7]}]


#Pmod Header JXADC
#Sch name = XA1_P
#set_property PACKAGE_PIN J3 [get_ports {vauxp6}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxp6}]
#Sch name = XA2_P
#set_property PACKAGE_PIN L3 [get_ports {vauxp14}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxp14}]
#Sch name = XA3_P
#set_property PACKAGE_PIN M2 [get_ports {vauxp7}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxp7}]
#Sch name = XA4_P
#set_property PACKAGE_PIN N2 [get_ports {vauxp15}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxp15}]
#Sch name = XA1_N
#set_property PACKAGE_PIN K3 [get_ports {vauxn6}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxn6}]
#Sch name = XA2_N
#set_property PACKAGE_PIN M3 [get_ports {vauxn14}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxn14}]
#Sch name = XA3_N
#set_property PACKAGE_PIN M1 [get_ports {vauxn7}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxn7}]
#Sch name = XA4_N
#set_property PACKAGE_PIN N1 [get_ports {vauxn15}]				
#set_property IOSTANDARD LVCMOS33 [get_ports {vauxn15}]


# VGA Definition
#set_property PACKAGE_PIN G19 [get_ports {red_o[0]}]
#set_property PACKAGE_PIN H19 [get_ports {red_o[1]}]
#set_property PACKAGE_PIN J19 [get_ports {red_o[2]}]
#set_property PACKAGE_PIN N19 [get_ports {red_o[3]}]
#set_property PACKAGE_PIN J17 [get_ports {green_o[0]}]
#set_property PACKAGE_PIN H17 [get_ports {green_o[1]}]
#set_property PACKAGE_PIN G17 [get_ports {green_o[2]}]
#set_property PACKAGE_PIN D17 [get_ports {green_o[3]}]
#set_property PACKAGE_PIN N18 [get_ports {blue_o[0]}]
#set_property PACKAGE_PIN L18 [get_ports {blue_o[1]}]
#set_property PACKAGE_PIN K18 [get_ports {blue_o[2]}]
#set_property PACKAGE_PIN J18 [get_ports {blue_o[3]}]
#set_property PACKAGE_PIN P19 [get_ports h_sync_o]
#set_property PACKAGE_PIN R19 [get_ports v_sync_o]
#set_property IOSTANDARD LVCMOS33 [get_ports {blue_o[3]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {blue_o[2]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {blue_o[1]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {blue_o[0]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {green_o[3]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {green_o[2]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {green_o[1]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {green_o[0]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {red_o[3]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {red_o[2]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {red_o[1]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {red_o[0]}]
#set_property IOSTANDARD LVCMOS33 [get_ports h_sync_o]
#set_property IOSTANDARD LVCMOS33 [get_ports v_sync_o]


# 7-Segment Definition - where V7 is decimal point
set_property PACKAGE_PIN V7 [get_ports {ss_o[7]}]
set_property PACKAGE_PIN U7 [get_ports {ss_o[6]}]
set_property PACKAGE_PIN V5 [get_ports {ss_o[5]}]
set_property PACKAGE_PIN U5 [get_ports {ss_o[4]}]
set_property PACKAGE_PIN V8 [get_ports {ss_o[3]}]
set_property PACKAGE_PIN U8 [get_ports {ss_o[2]}]
set_property PACKAGE_PIN W6 [get_ports {ss_o[1]}]
set_property PACKAGE_PIN W7 [get_ports {ss_o[0]}]
set_property PACKAGE_PIN U2 [get_ports {ss_sel_o[0]}]
set_property PACKAGE_PIN U4 [get_ports {ss_sel_o[1]}]
set_property PACKAGE_PIN V4 [get_ports {ss_sel_o[2]}]
set_property PACKAGE_PIN W4 [get_ports {ss_sel_o[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_o[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_sel_o[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_sel_o[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_sel_o[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {ss_sel_o[3]}]


# LED Definition - from L1:left to U16:right
#set_property PACKAGE_PIN L1 [get_ports {led_o[15]}]
#set_property PACKAGE_PIN P1 [get_ports {led_o[14]}]
#set_property PACKAGE_PIN N3 [get_ports {led_o[13]}]
#set_property PACKAGE_PIN P3 [get_ports {led_o[12]}]
#set_property PACKAGE_PIN U3 [get_ports {led_o[11]}]
#set_property PACKAGE_PIN W3 [get_ports {led_o[10]}]
#set_property PACKAGE_PIN V3 [get_ports {led_o[9]}]
#set_property PACKAGE_PIN V13 [get_ports {led_o[8]}]
#set_property PACKAGE_PIN V14 [get_ports {led_o[7]}]
#set_property PACKAGE_PIN U14 [get_ports {led_o[6]}]
#set_property PACKAGE_PIN U15 [get_ports {led_o[5]}]
#set_property PACKAGE_PIN W18 [get_ports {led_o[4]}]
#set_property PACKAGE_PIN V19 [get_ports {led_o[3]}]
#set_property PACKAGE_PIN U19 [get_ports {led_o[2]}]
#set_property PACKAGE_PIN E19 [get_ports {led_o[1]}]
set_property PACKAGE_PIN U16 [get_ports {led_o}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[15]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[14]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[13]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[12]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[11]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[10]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[9]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[8]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[7]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[6]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[5]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[4]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[3]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[2]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {led_o[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {led_o}]


# Switch Definition - from R2:left to V17:right
#set_property PACKAGE_PIN R2 [get_ports {sw_i[15]}]
#set_property PACKAGE_PIN T1 [get_ports {sw_i[14]}]
#set_property PACKAGE_PIN U1 [get_ports {sw_i[13]}]
#set_property PACKAGE_PIN W2 [get_ports {sw_i[12]}]
#set_property PACKAGE_PIN R3 [get_ports {sw_i[11]}]
#set_property PACKAGE_PIN T2 [get_ports {sw_i[10]}]
#set_property PACKAGE_PIN T3 [get_ports {sw_i[9]}]
#set_property PACKAGE_PIN V2 [get_ports {sw_i[8]}]
#set_property PACKAGE_PIN W13 [get_ports {sw_i[7]}]
#set_property PACKAGE_PIN W14 [get_ports {sw_i[6]}]
#set_property PACKAGE_PIN V15 [get_ports {sw_i[5]}]
#set_property PACKAGE_PIN W15 [get_ports {sw_i[4]}]
#set_property PACKAGE_PIN W17 [get_ports {sw_i[3]}]
#set_property PACKAGE_PIN W16 [get_ports {sw_i[2]}]
#set_property PACKAGE_PIN V16 [get_ports {sw_i[1]}]
#set_property PACKAGE_PIN V17 [get_ports {sw_i[0]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[15]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[14]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[13]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[12]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[11]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[10]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[9]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[8]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[7]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[6]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[5]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[4]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[3]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[2]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[1]}]
#set_property IOSTANDARD LVCMOS33 [get_ports {sw_i[0]}]


# Push-Button Definition - T18:up;U17:down;W19:left;T17:right
set_property PACKAGE_PIN U17 [get_ports {pb_i[0]}]
set_property PACKAGE_PIN W19 [get_ports {pb_i[1]}]
set_property PACKAGE_PIN T17 [get_ports {pb_i[2]}]
set_property PACKAGE_PIN T18 [get_ports {pb_i[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pb_i[0]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pb_i[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pb_i[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pb_i[3]}]


# Reset-Button Definition - Middle button
set_property PACKAGE_PIN U18 [get_ports rst_i]
set_property IOSTANDARD LVCMOS33 [get_ports rst_i]


# master clock configuration at 100MHz
set_property PACKAGE_PIN W5 [get_ports clk_i]
set_property IOSTANDARD LVCMOS33 [get_ports clk_i]
create_clock -add -period 10.000 -name mclk -waveform {0.000 5.000} [get_ports clk_i]
set_property CONFIG_VOLTAGE 3.3 [current_design]
set_property CFGBVS VCCO [current_design]
